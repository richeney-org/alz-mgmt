...
DESCRIPTION
  ...
  validation {
    error_message = "All subscription ids must be valid UUIDs or empty/null."
    condition     = alltrue([for v in var.subscription_placement : v.subscription_id == "" || v.subscription_id == null || can(regex("^[a-f\\d]{4}(?:[a-f\\d]{4}-){4}[a-f\\d]{12}$", v.subscription_id))])
  }
  validation {
    error_message = "Each subscription ID can only be specified once in the subscription_placement variable."
    condition     = length([for v in var.subscription_placement : v.subscription_id if v.subscription_id != "" && v.subscription_id != null]) == length(distinct([for v in var.subscription_placement : v.subscription_id if v.subscription_id != "" && v.subscription_id != null]))
  }
}
